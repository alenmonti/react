    ...
    return (
        <aside className={`fixed right-0 top-0 w-full h-full p-8 lg:w-[30%] bg-white z-50 transform ${openCart ? "translate-x-0" : "translate-x-full"} shadow-xl transition-transform duration-500 ease-in-out`}>
            <div className="w-full h-[80px] flex justify-between items-center border-b-[1px] border-gray-300">
                <h2 className="text-lg font-semibold">{`Cart (${products.length})`}</h2>
                <button onClick={(e) => setOpenCart(false)} className="">
                        ...
                        <div key={index} className="w-full h-[110px] flex justify-between items-center flex-shrink-0">
                            ...
                            <div className="h-[60%] flex flex-col justify-between w-2/3">
                                <div className="flex justify-between w-full items-start">
                                    ...
                                    <IoClose onClick={e => setProducts(prev => prev.splice(index, 1))} size={15} className="text-gray-500"/>
                                </div>
                                <div className="flex justify-between w-full text-xs font-semibold items-center">
                                    <div className="flex justify-between w-16 border-[1px] border-gray-300 px-[6px] py-1 text-sm font-normal rounded-sm">
                                        <button className="w-4"><BsDash className="mx-auto"/></button>
                                        <p>3</p>
                                        <button className="w-4"><BsPlus className="mx-auto"/></button>
                                    </div>
